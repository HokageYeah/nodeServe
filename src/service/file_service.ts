...
interface File_DBServiceCls<T> {
    ...
    queryFile(userid: string): Promise<[RowDataPacket[] | RowDataPacket[][] | OkPacket | OkPacket[] | ResultSetHeader, FieldPacket[]]>;
}
class File_DBService<T> implements File_DBServiceCls<T>{
    async queryFile(userid: string): Promise<[ResultSetHeader | RowDataPacket[] | RowDataPacket[][] | OkPacket | OkPacket[], FieldPacket[]]> {
        const connect = await connectionMysql();
        // 拼接sql语句
        const sql = `SELECT * FROM user_avatar WHERE userid = ?`;
        const createMoment = await connect.execute<ResultSetHeader>(sql, [userid]);
        connect.release();
        return createMoment;
    }
